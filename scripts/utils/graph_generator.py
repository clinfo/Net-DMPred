...
def make_nodes_undirected(graph: DataFrame) -> DataFrame:
    graph_prime = graph.rename(columns={"edge1": "edge2", "edge2": "edge1"})
...
def convert_gene_ids(graph: DataFrame, hgnc: DataFrame) -> DataFrame:
    dic = hgnc.reset_index(drop=True).to_dict("records")
    dic = {element["Symbol"]: element["HGNC_ID"] for element in dic}
...
class GraphGeneration(object):
    ...
    def save_graph(
        ...
        self.graph_data = self.graph_data[["edge1", "rel", "edge2"]]
        self.graph_data = clean_graph(self.graph_data)
        print("node num:",len(self.vertices_dic))
        print(self.graph_data.shape)
        print(self.graph_data.head())

        self.graph_data.to_csv(
            graph_location,
            sep="\t",
            index=False,
            header=False,
