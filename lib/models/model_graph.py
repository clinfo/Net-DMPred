...
class GraphNet(torch.nn.Module):
    def __init__(
        ...
        self, node_num: int, adj_num: int, num_node_feature: int = 32
    ) -> None:
        super(GraphNet, self).__init__()

        self.adj_num = adj_num
        self.num_layer = 2
        self.embed = torch.nn.Embedding(node_num, num_node_feature)
        ...
        self.linear_last = torch.nn.Linear(
            num_node_feature * (self.num_layer), num_node_feature
        )  # without input
        ...
        for _ in range(self.num_layer):
            ...
            for _ in range(self.adj_num):  # Types of edges
                nn = torch.nn.Sequential(
                    torch.nn.Linear(num_node_feature, num_node_feature),
                    torch.nn.ELU(),
                    torch.nn.Linear(num_node_feature, num_node_feature),
                )
